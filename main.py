...
def send_message(event=None):
    ...
    chat_log.insert(tk.END, "Sen: " + user_input + "\n")
    ...
    if response:
        ...
        chat_log.insert(tk.END,
                        "Gpd-i: Bu soruya nasıl cevap vereceğimi bilmiyorum. Lütfen bana nasıl cevap vereceğimi öğret.\n")
        chat_log.config(state=tk.DISABLED)
        chat_log.yview(tk.END)

        new_response = simpledialog.askstring("Cevap Öğret", "Bu soruya nasıl cevap vermeliyim?")
        if new_response:
            if user_input in database:
                database[user_input].append(new_response)
            else:
                database[user_input] = [new_response]
            save_database(database)
            chat_log.config(state=tk.NORMAL)
            chat_log.insert(tk.END, "Gpd-i: Teşekkürler! Şimdi öğrendim.\n")
            chat_log.config(state=tk.DISABLED)
            chat_log.yview(tk.END)
